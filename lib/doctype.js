    ...
    if (_configuration) {
        switch (_configuration.parser) {
            ...
            case 'markdown':
                ...
                var code_line = _configuration.code_line | '^ {4,}(?!.*(\\*|\\+|\\-|\\d+)).*';
...
function fromLanguage(id) {
    switch (id) {
        ...
        case 'diff':
            return null;
        case 'yaml':
            return new Standard_1.default({ comment_line: '#', string_inline: '["\']', string_quote: '\\' });
        case 'toml':
        case 'dockerfile':
        case 'shellscript':
        case 'r':
        ...
        case 'rst':
        case 'markdown':
            return new Markdown_1.default({ code_start: '\`\`\`', code_end: '', code_line: '^ {4,}(?!.*(\\*|\\+|\\-|\\d+)).*' });
        case 'madoko':
            return new Markdown_1.default({ code_start: '\`\`\`', code_end: '', code_line: '^ {4,}(?!.*(\\*|\\+|\\-|\\d+)).*'  });
        case 'asciidoc':
        ...
        case 'diet':
            return new Plaintext_1.default();
        case 'perl':
            return new Standard_1.default({ comment_start: '^=\\w+', comment_end: '^=cut', comment_line: '#' });
        case 'perl6':
